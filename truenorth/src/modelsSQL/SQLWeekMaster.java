...
import helperUtilities.ConstantsWeekMaster;
...
public class SQLWeekMaster extends SQLQueryEngine {

	public SQLWeekMaster(Connection conn, SQLQueryConfig queryConfig) {
		super(conn, queryConfig);
	}

	public <T> List<T> fetchAllData (Map<Integer, DSCustomizable> params, Object errDisplayObj) {
		List<DSWeekMaster> results = null;
		...
		try {
			...
			try {
				...
				results = new ArrayList<DSWeekMaster>();
				while (resultSet.next()) results.add(new DSWeekMaster(
						resultSet.getString(ConstantsWeekMaster._FISCAL_WEEK)));
