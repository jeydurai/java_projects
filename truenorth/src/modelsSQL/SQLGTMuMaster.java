...
import helperUtilities.ConstantsGTMuMaster;
...
public class SQLGTMuMaster extends SQLQueryEngine{

	public SQLGTMuMaster(Connection conn, SQLQueryConfig queryConfig) {
		super(conn, queryConfig);
	}

	public <T> List<T> fetchAllData (Map<Integer, DSCustomizable> params, Object errDisplayObj) {
		List<DSGTMuMaster> results = null;
		...
		try {
			...
			try {
				...
				results = new ArrayList<DSGTMuMaster>();
				while (resultSet.next()) results.add(new DSGTMuMaster(
						resultSet.getString(ConstantsGTMuMaster._GTMu)));
